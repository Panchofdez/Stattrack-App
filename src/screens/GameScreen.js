...
import {StyleSheet, View, SafeAreaView,FlatList, Dimensions, TouchableOpacity} from 'react-native';
import {Text, Button, ListItem, Card, Overlay, SearchBar} from 'react-native-elements';
							...
							rightIcon={
								<View style={{flexDirection:'row'}}>
									<Button
										...
										buttonStyle={{borderRadius:50, backgroundColor:'#02a1e6', marginHorizontal:2, padding:10}}
										onPress={()=>toggle()}
									/>
									<Button
										title='Reset'
										...
										buttonStyle={{borderRadius:50, backgroundColor:'#02a1e6', padding:14}}
										onPress={()=>reset()}
									/>
								</View>
							}
				...
				renderItem={({item})=>{
					...
					return(
						<View style={styles.btnContainer}>
						 	<Text style={{alignSelf:'center', color:item.color}} h4Style={{fontSize:28}} h4>{gameState[name]}</Text>
						 	<Button
						 		title={item.title}
						 		titleStyle={{fontSize:14}}
						 		buttonStyle={{height:height*0.06, backgroundColor:item.color,borderRadius:50}}
						 		containerStyle={{marginHorizontal:3}}
						 		raised
						 		onPress={()=>setGameState(prevState=>{
						 			return {...prevState , [name] : prevState[name] + 1}
						 		})}
						 		onLongPress={()=>setGameState(prevState=>{
						 			if (prevState[name]=== 0){
						 				return prevState
						 			}else{
						 				return {...prevState , [name] : prevState[name] - 1}
			...
			<Overlay isVisible={visible} onBackdropPress={toggleOverlay} overlayStyle={styles.overlay}>
				<View>
				    ...
				    <Button 
				    	...
				    	buttonStyle={{borderRadius:50, backgroundColor:'#02a1e6', height:55}} 
				    	onPress={()=>{

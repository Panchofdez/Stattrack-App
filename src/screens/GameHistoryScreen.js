...
const GameHistoryScreen = ({navigation})=>{
	...
	const filterGames = (gameArr,name)=>{
		...
		if(name==""){
			...
			const newArr = gameArr.filter(game=>game.player.name.toLowerCase().indexOf(nameLower) !== -1 || game.name.toLowerCase().indexOf(nameLower)!== -1);
			return newArr;
		}
	}
	const fetch = async()=>{
		...
		return(
			<SafeAreaView style={styles.container}>
				<FlatList
						...
						<View>
						    ...
						    {games.length===0 &&(
								<TouchableOpacity activeOpacity={0.85}>
									...
									>	
										<View style={styles.titleContainer}><Text style={styles.title}>No games</Text></View>
									</Card>
								</TouchableOpacity>
					...
					renderItem={({item})=>{
						return (
							<TouchableOpacity onPress={()=>navigation.navigate('GameShow', {game:item})} activeOpacity={0.85}>
								<Card
									containerStyle={styles.card}
								>	
									<View style={styles.titleContainer}><Text style={styles.title}>{item.name}</Text></View>
									<Text style={styles.text}>{dayjs(item.date).format('DD MMM YYYY')}</Text>
									<ListItem 
